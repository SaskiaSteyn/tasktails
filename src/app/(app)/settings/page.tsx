...
export default async function SettingsPage() {
  ...
  return (
    ...
    >
      ...
      {/* INF-22 — the handoff's 280px section list. Plain in-page anchors, not
          a filter: every section is on this one screen and always has been, so
          a link that scrolls to it is the whole of what the column has to do,
      ...
      >
        {[
          ...
          <a
            key={href}
            href={href}
            className="rounded-[11px] px-[14px] py-3 text-[14px] font-bold text-ink-soft transition-colors duration-120 hover:bg-warm hover:text-ink"
          >
            {label}
          </a>
        ))}
