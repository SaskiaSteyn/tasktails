...
| NFR-TASK-2 | **Earning cooldown** (revised #224, replaces the retired 300 coin / 500 XP daily cap) — after **3 rewarded whole-task completions** (subtasks do not count toward the window), task completions stop earning coins and XP for a cooldown of **20–60 minutes**. The length is tier-weighted: `round5(20 + (avgTier − 1) / 4 × 40)` minutes over the mean `complexityTier` of the 3 tasks — three Trivial tasks → 20 min, three Epic tasks → 60 min. During the cooldown a completion still marks the task done and still counts toward the streak and achievements, but grants nothing. The window and cooldown are duration-based (no calendar-day reset) and clear lazily on the next completion after the cooldown lifts. Design + telemetry: `design_handoff/ADDENDUM-earning-cooldown.md`. |
...
>
> **Superseded (#224):** NFR-TASK-2 is now the earning cooldown, not a hard daily XP ceiling, so this "day two" guarantee no longer holds by construction — a determined participant can cross Rare-tier thresholds on day one. Saskia's call was to leave the XP curve (`design_handoff/ADDENDUM-xp-curve.md`, v5) unchanged for now and revisit only if the #224 earning telemetry shows tier exposure running ahead of §3.7's schedule.
>
