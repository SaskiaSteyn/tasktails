...
| TASK-05 | Complete task action — checkbox or button; triggers reward animation and coin/XP update | Frontend | Done — `src/components/tasks/task-row.tsx` + `task-list.tsx`. **Frontend-only by explicit scope decision**: TASK-11 (`POST /api/tasks/[id]/complete`) and the entire ECO-01..05 chain it depends on (6 backend tickets) don't exist, so completion state is local-only — toggling is real and reverts on reload rather than persisting. **Tap targets split**, also by explicit decision: the checkbox completes/uncompletes in place; the rest of the row (title through coins) still links to TASK-03's edit screen, since the mock's single whole-row tap-to-complete can't serve both interactions at once now that both exist. Reward pop ("+35 · +45 XP") plays via a new `task-reward-float` keyframe in `globals.css` on completing (not on un-completing), respects `prefers-reduced-motion` (the existing global media query already covers it). One shared "not saved" notice for the whole list (auto-dismisses after 3s), not one per row. Verified live end-to-end after ruling out two false alarms — a stale Turbopack console error from hours earlier in the same browser tab's scrollback, and a test-timing artifact where my own checks ran past the notice's 3s window — neither was a real bug once isolated in a fresh tab/server |
