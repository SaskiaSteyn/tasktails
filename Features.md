...
| SUB-03 | Complete subtask action — triggers proportional reward | Frontend | Done — folded into `src/components/tasks/subtask-list.tsx` (SUB-01/02's file). Each incomplete row's checkbox is now a real button; a done row's is disabled rather than toggling back — same forward-only rule TASK-05/11 use, for the same reason (undoing a grant correctly means unwinding a streak day/cap/level, out of scope here). **No `POST /api/tasks/[id]/subtasks/[subId]/complete` yet** — SUB-05 is a separate, unbuilt ticket, so unlike TASK-05 (which calls TASK-11's real endpoint) this doesn't attempt a fetch at all — same "don't call an endpoint that doesn't exist" decision TASK-02 made ahead of TASK-08. A click just surfaces "Not connected yet — completing a subtask needs SUB-05." below the list; the row's `completedAt`/strikethrough/coin-share stay exactly as they were, no optimistic update, since there's nothing real to reflect yet. Verified live: clicking an incomplete row's checkbox shows the notice and leaves the DB untouched (checked directly); clicking a done row's checkbox does nothing (disabled) |
