...
| INF-14 | WCAG AA compliance pass — contrast ratios, keyboard navigation, focus states, colour-independent signals | Frontend | To Do — contrast is now measured with **BPCA** (bridge for WCAG 2 contrast using APCA), not plain WCAG 2: `src/lib/contrast.ts`, ported from `bridge-pca` 0.1.6 and checked by `scripts/verify-bpca.ts`. That re-opened four text tokens (`ink-faint`, `amber-text`, `sage-text`, `urgency-text`) which were tuned to land exactly on 4.5:1 under the old formula and now fall to ~3.6–3.7 on the three darker surfaces; candidate replacements are in the `globals.css` audit block. Keyboard nav, focus states and colour-independent signals still outstanding |
